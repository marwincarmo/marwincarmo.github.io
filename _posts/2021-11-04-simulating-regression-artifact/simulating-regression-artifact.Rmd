...
Recently I came across a publication [@farmus2019] where the authors outline a statistical artifact that may arise when researchers add continuous predictors correlated with baseline scores as covariates in regression models of pretest-posttest analysis. This effect is known widely as Lord's Paradox.
...
I'll borrow some parameter values used in @eriksson2014 because later in this post I will re run their simulation study of a estimate of the regression artifact quantity.
...
```{r, echo=FALSE, out.width="50%", fig.align='center',fig.cap="Linear version of the model showing pretest scores (Pre) as a mediator between Group (G) and posttest scores (Post). Adapted from @pearl2016"}
...
\end{align}
```
Our first step is to create a function that takes as arguments the number of runs, sample size and the population value of coefficient $b$. Then, we can store the *p*-value of each analysis and count afterwards how many were less than the the alpha level.
