...
data <- as.data.frame(matrix(rnorm(40*5), nrow = 40, ncol = 5))
colnames(data) <- c("y", paste0("x", 1:4))

fit <- lm(y ~., data = data, na.action = "na.fail")
model_dredge <- MuMIn::dredge(fit, beta = FALSE, evaluate = TRUE, rank = "AICc")
sel <- MuMIn::get.models(model_dredge, subset = 1)[[1]]
s <- summary(sel)
```

```{r weight}
exp((-1/2)*model_dredge[1]$delta)/sum(exp((-1/2)*model_dredge$delta))
...
sum(model_dredge$weight * ifelse(is.na(model_dredge$x1), 0, model_dredge$x1))

mean(model_dredge$x1, na.rm=TRUE)

```
```{r}
(s$coefmat.full[,3]^2)*n
```

```{r}
models <- MuMIn::get.models(model_dredge, subset = TRUE)
mapped <- purrr::map_dfr(models, broom::tidy, .id = "model")

arruma_coluna <- function(.x) {
  tidyr::replace_na(.x, 0)
}

mapped_with_zeros <- mapped |> 
  #dplyr::filter(term != "(Intercept)") |> 
  tidyr::pivot_longer(cols = 3:6, names_to = "statistic", values_to = "value") |> 
  tidyr::pivot_wider(names_from = "term", values_from = "value") |> 
  dplyr::mutate(dplyr::across(is.double, arruma_coluna)) |> 
  tidyr::pivot_longer(cols = 3:7, names_to = "term", values_to = "value") |> 
...
se_x1 <- mapped_with_zeros |> 
  dplyr::filter(term == "x1") |> 
  dplyr::pull(std.error)

vars_x1 <- (se_x1^2)*n
b_chapeu_x1 <- mapped_with_zeros |> 
  dplyr::filter(term == "x1") |> 
  dplyr::pull(estimate)
b_full_x1 <- avg.mod$coefficients[1,2]

var_b_til_x1 <- sum(model_dredge$weight*(vars_x1 + (b_chapeu_x1 - b_full_x1)^2))

cis_x1 <- c('low' = b_full_x1 - 2.03*sqrt(var_b_til_x1),
            'high' = b_full_x1 + 2.03*sqrt(var_b_til_x1))
