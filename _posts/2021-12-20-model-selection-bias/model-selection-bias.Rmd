...
Note that $\beta_{yx}$ is not the same as $\beta_{yx\cdot z}$. If we do not have a model specified prior to data collection and analysis, it is not clear which definition of regression parameter for $x$ we're trying to estimate, if $\beta_{yx\cdot z}$, exemplified on \@ref(eq:beta12) or if $\beta_{yx}$. Therefore, the definition of $x$ depends on the model in which it is placed.
