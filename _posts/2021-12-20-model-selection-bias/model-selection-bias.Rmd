...
where $\hat{\sigma_{\varepsilon}}$ is an estimate of the residual standard deviation, $s_x$ is the sample standard deviation of $x$, and $r^2_{xy}$ is the square of the sample correlation between $x$ and $z$. If the standard error for a regression coefficient is large, it means that its distribution will be more dispersed. So, from this equation we identify as crucial parameters for wider sampling distribution: larger residual variance, less variance in $x$, small sample size, and, stronger correlation between regressors.
...
Because we'll reuse the same code in other examples, t 
